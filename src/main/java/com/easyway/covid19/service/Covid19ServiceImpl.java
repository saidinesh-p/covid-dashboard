...
public class Covid19ServiceImpl implements Covid19Service {
    ...
    public Response addRecordsDetails(String recordsInfo) throws IOException {
        ...
        if (StringUtils.isBlank(recordsObject.getState())){
            ...
            if (StringUtils.isNotBlank(exception) && exception.contains("Duplicate entry")) {
                LOGGER.info("Exception occured due to uniqueKeyconstraint  :{}" + e);
                String error = "The state already exists,Please try with different state";
                return processErrorResponseCode(new RecordsResponse(), ErrorCodeEnum.DUPLICATE_ENTRY, error, LOGGER);
            }
            LOGGER.info("Exception occured while communicating with Database with exception :{} " + e);
    ...
    public Response getAllRecordDetails() {
        ...
        List<Records> recordsObjectList = new ArrayList<>();
        try{
            LOGGER.info("#########get ALL RecordDetails ####") ;
            List<Records> recordsList = recordsDao.getAllRecordDetails();
            recordsResponse = getRecordsObjectResponse(recordsList);
            return processResponse(recordsResponse);
